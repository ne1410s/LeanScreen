...
namespace Av.Rendering.Ffmpeg
{
    internal sealed unsafe class StreamSourceDecoder : IDecoder
    {
        ...
        public StreamSourceDecoder(ISimpleReadStream stream)
        {
            ...
            _pFormatContext->pb = customInputStreamContext;
            _pFormatContext->seek2any = 1;
            ffmpeg.avformat_open_input(&pFormatContext, string.Empty, null, null).ThrowExceptionIfError();
            ffmpeg.av_format_inject_global_side_data(_pFormatContext);
            // ----- End Stream stuffs
