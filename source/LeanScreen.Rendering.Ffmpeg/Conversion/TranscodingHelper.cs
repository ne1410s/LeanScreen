// <copyright file="TranscodingHelper.cs" company="ne1410s">
...
/// Helper.
/// </summary>
public static class TranscodingHelper
{
    /// <summary>
    /// Tcode.
    /// </summary>
    ...
    {
        private unsafe struct FilteringContext
        {
        ...
        private AVFormatContext* ofmt_ctx;

        private FilteringContext* filter_ctx;

        private StreamContext* stream_ctx;
        ...
        private int open_input_file(string filename)
        {
            ...
            stream_ctx = (StreamContext*)ffmpeg.av_malloc_array(ifmt_ctx->nb_streams, (ulong)sizeof(StreamContext));
            if (stream_ctx == null)
        ...
        private int open_output_file(string filename)
        {
            AVStream* out_stream;
            AVStream* in_stream;
            ...
            for (i = 0; i < ifmt_ctx->nb_streams; i++)
            {
                ...
                else
                {
                    ...
                    if (ret < 0)
                    {
                        Console.WriteLine("Copying parameters for stream #%u failed", i);
                        return ret;
                    }
        ...
        private int init_filter(FilteringContext* fctx, AVCodecContext* dec_ctx,
            AVCodecContext* enc_ctx, string filter_spec)
        {
            ...
            else if (dec_ctx->codec_type == AVMediaType.AVMEDIA_TYPE_AUDIO)
            {
                ...
                //{
                //    dec_ctx->ch_layout =
                //        (ulong)ffmpeg.av_channel_layout_default(dec_ctx->ch_layout, 1);
                //}
        ...
        private int encode_write_frame(AVFrame* filt_frame, uint stream_index, int* got_frame)
        {
            int ret;
            int got_frame_local;
            AVPacket enc_pkt;
            ...
            Console.WriteLine("Muxing frame");
            /* mux encoded frame */
            ret = ffmpeg.av_interleaved_write_frame(ofmt_ctx, &enc_pkt);
            return ret;
        }

        private int filter_encode_write_frame(AVFrame* frame, uint stream_index)
        {
            int ret;
            AVFrame* filt_frame;

            Console.WriteLine("Pushing decoded frame to filters");
            /* push the decoded frame into the filtergraph */
            ret = ffmpeg.av_buffersrc_add_frame_flags(filter_ctx[stream_index].buffersrc_ctx,
                frame, 0);
            if (ret < 0)
            {
            ...
            while (true)
            {
                ...
                Console.WriteLine("Pulling filtered frame from filters");
                ret = ffmpeg.av_buffersink_get_frame(filter_ctx[stream_index].buffersink_ctx,
                    filt_frame);
                if (ret < 0)
                {
        ...
        /// <returns></returns>
        private int flush_encoder(uint stream_index)
        {
            int ret;
            int got_frame = 0;
            ...
            while (true)
            {
                Console.WriteLine($"Flushing stream #{stream_index} encoder");
            ...
            while (true)
            {
                ...
                if (filter_ctx[stream_index].filter_graph != null)
                {
                    ...
                    if (type == AVMediaType.AVMEDIA_TYPE_VIDEO)
                        ret = ffmpeg.avcodec_send_frame(stream_ctx[stream_index].dec_ctx, frame);


                    ////ret = ffmpeg.avcodec_decode_video2(stream_ctx[stream_index].dec_ctx, frame,
                    ////    &got_frame, &packet);
                    else
                        ret = ffmpeg.avcodec_decode_audio4(stream_ctx[stream_index].dec_ctx, frame,
                            &got_frame, &packet);
                    if (ret < 0)
                    {
