...
public class BulkMediaUtilsTests
{
    ...
    public async Task Ingest_WithFiles_ProcessedAsExpected()
    {
        ...
        _ = targetDir.CreateSubdirectory("5e");
        ...
        new FileInfo(expectSaveTo).Length.ShouldNotBe(0);
        new FileInfo($"{sourceDir}/sample.flv").Exists.ShouldBeFalse();
    ...
    public async Task Ingest_NewlySecuredFile_CapsExpected()
    {
        ...
        // Act
        _ = await sourceDir.Ingest([9, 0, 2, 1, 0], targetDir.FullName, applySnap: true);

        // Assert
        new FileInfo($"{targetDir}/c1/{name}").Exists.ShouldBeTrue();
        sourceDir.Delete(true);
        targetDir.Delete(true);
    }
    ...
    public async Task Ingest_PlainSourceWithUncappedAnalogue_CapsExpected()
    {
        ...
        var sourceDir = ogDir.CreateSubdirectory(Guid.NewGuid().ToString());
        var targetDir = ogDir.CreateSubdirectory(Guid.NewGuid().ToString());
        _ = targetDir.CreateSubdirectory("c1");
        File.Copy($"{ogDir}/sample.flv", $"{sourceDir}/sample.flv");
    ...
    public async Task Ingest_VaryingPurge_OnlyPurgeRemovedNonPertinentFile(bool purge)
    {
        ...
        // Assert
        new FileInfo(nonMediaPath).Exists.ShouldBe(!purge);
    }
    ...
    public async Task ApplyCaps_WithProgress_InvokesAction()
    {
        ...
        _ = targetDir.CreateSubdirectory("c1");
        _ = targetDir.CreateSubdirectory("5e");
        _ = targetDir.CreateSubdirectory("c4");
        ...
        // Act
        _ = await BulkMediaUtils.ApplyCaps([9, 0, 2, 1, 0], targetDir.FullName, onProgress: mockProgress.Object);
    ...
    public async Task ApplyCaps_HasSomeWorkToDo_AppliesCaps()
    {
        ...
        // Act
        _ = await BulkMediaUtils.ApplyCaps([9, 0, 2, 1, 0], targetDir.FullName);

        // Assert
        new FileInfo($"{targetDir}/c4/{expectedName}").Exists.ShouldBe(true);
        targetDir.Delete(true);
    }
    ...
    [Theory]
    [InlineData("literally-anything", ".*empty.*")]
    public void GetRepo_VaryingParam_ThrowsExpected(string requestName, string expectedError)
    {
        ...
        // Assert
        act.ShouldThrow<Exception>().Message.ShouldMatch(expectedError);
    }
}
